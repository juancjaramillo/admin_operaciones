...
GLOBAL $db,$CFG, $ME;
	...
	if(!isset($frm["id_vehiculo"]) || $frm["id_vehiculo"]=="") $frm["id_vehiculo"]="0";
	if(!isset($frm["fecha_desde"])) $frm["fecha_desde"]=date("Y-m-d H:i:s",strtotime("today 6:00AM"));
	if(!isset($frm["fecha_hasta"])) $frm["fecha_hasta"]=date("Y-m-d H:i:s");
	...
	$qVehiculos=$db->sql_query("
		SELECT v.id, v.codigo || '/' || v.placa as label, v.idgps
		FROM vehiculos v
