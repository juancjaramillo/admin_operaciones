...
//	$origen=preg_replace("/[^í]í(.)/","\\1",$origen);
...
//>STXT1=15;N1=0116.0;T2=14;N2=0118.0;EV30??????????+???????+????????????????;TX0;%4,2331,6.45,0,1,1,0,0;RT112321<
function interpretar_trama_spF8($origen,$imei,$id_vehi){
 echo "DIVISON";
 // $imei = hexdec($imei);
 
 //echo "INTERPRETAR".substr($origen,54,220);
 // $vectorcaptura= substr($origen,54,220);  
//  $vectorforsplit= Hex2String($vectorcaptura); 
 ...
 // substr($array[2],6,15);
  ...
  echo "DIVISON".$division[0]." ".$division[1]." ".$division[2]." ".$division[3]." ".$division[4][1]." T1".hexdec("16")." N1".hexdec("03ff")." N2".hexdec("0118")."\n";
//  $s= str_replace(',', '', substr($trama[6],1,2));
//echo "QQQ".  $s."\n";
	$arreglo=array(); 	
...
function insertar_registro_gps_F8($arreglo){
	...
	echo "INSERT INTO sensores (id_vehi, temperatura_sonda1, temperatura_sonda2, nivel_combustible_sonda1, nivel_combustible_sonda2, sensor_peso,imei,tiempo)
		VALUES('".$arreglo['id_vehi']."','".$arreglo['temperatura_sonda1']."','".$arreglo['temperatura_sonda2']."','".$arreglo['nivel_combustible_sonda1']."','".$arreglo['nivel_combustible_sonda2']."','".$arreglo['sensor_peso']."','".$arreglo['imei']."','".$fecha_proceso."')";
		
		
		
		if (arreglo['id_vehi']!="???"){
		$fecha_proceso= date("Y-m-d H:i:s");
		$sql= "INSERT INTO sensores (id_vehi, temperatura_sonda1, temperatura_sonda2, nivel_combustible_sonda1, nivel_combustible_sonda2, sensor_peso,imei,tiempo)
		VALUES('".$arreglo['id_vehi']."','".$arreglo['temperatura_sonda1']."','".$arreglo['temperatura_sonda2']."','".$arreglo['nivel_combustible_sonda1']."','".$arreglo['nivel_combustible_sonda2']."','".$arreglo['sensor_peso']."','".$arreglo['imei']."','".$fecha_proceso."')";
		$qid=$db->sql_query($sql);
...
//			$posicion_nueva=$geometry_vehi;
			$diferencia_horas=abs(strtotime($arreglo["tiempo"])-time())/60/60;
			if($diferencia_horas>24){
#	error_log("Registro inválido: ". print_r($arreglo,true));
			error_log("Registro inválido: " . print_r("Tiempo->".$arreglo['tiempo']." vehiculo->".$arreglo['id_vehiculo']." longitud->".$arreglo['longitud']." latitud->".$arreglo['latitud']." \n",true));
				return;
			}
		}
...
/*
	...
	elseif($arreglo["evento"]=="25"){//Pánico
