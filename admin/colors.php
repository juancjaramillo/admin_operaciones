...
$defaultColor=$frm["color"];

$colores = array("FBEFEF","FBF2EF","FBF5EF","FBF8EF","FBFBEF","F8FBEF","F5FBEF","F2FBEF","EFFBEF","EFFBF2","EFFBF5","EFFBF8","EFFBFB","EFF8FB","EFF5FB","EFF2FB","EFEFFB","F2EFFB","F5EFFB","F8EFFB","FBEFFB","FBEFF8","FBEFF5","FBEFF2","FFFFFF",
...
$string_colors = "<table>\n";
$i=0;
foreach($colores as $hexa)
{
	if($i==0)
		 $string_colors.="<tr>\n";

	$string_colors.="<td bgColor='".$hexa."'";
	$string_colors .= " onMouseOver=\"View('" . $hexa . "')\"";
	if(nvl($_GET["submit"],0)) $string_colors .= " onClick=\"Update('$hexa','" . $hexa . "')\"";
	else $string_colors .= " onClick=\"Update('$hexa','" . $hexa . "')\""; 
	$string_colors .=" height=\"10px\" width=\"10px\"></td>";
...
/*
$color=explode(",",$frm["color"]);
$defaultColor=str_pad(dechex($color[0]),2,'0') . str_pad(dechex($color[1]),2,'0') . str_pad(dechex($color[2]),2,'0');

$string_colors = "\n";

for($i=0;$i<=255;$i=$i+51){
	$red=str_pad(dechex($i),2,'0');
	$string_colors .=  "\n<tr>";
	for($j=0;$j<=102;$j=$j+51){
		$green=str_pad(dechex($j),2,'0');
		for($k=0;$k<=255;$k=$k+51){
			$blue=str_pad(dechex($k),2,'0');
			$string_colors .= "\n<td bgcolor=\"" . $red . $green . $blue . "\"";
			$string_colors .= " onMouseOver=\"View('" . $red . $green . $blue . "')\"";
			if(nvl($_GET["submit"],0)) $string_colors .= " onClick=\"Update('$i,$j,$k','" . $red . $green . $blue . "')\"";
			else $string_colors .= " onClick=\"Update('$i $j $k','" . $red . $green . $blue . "')\""; 
			$string_colors .= " height=\"10px\" width=\"10px\"></td>";
		}
	}
	$string_colors .=   "\n</tr>";
...
include($CFG->templatedir . "/color_form.php");
?> 
