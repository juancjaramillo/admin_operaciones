...
// ---------- ATRIBUTOS          ----------------
	...
	$atributo->set("qsQuery","
			SELECT mov.id, m.fecha||'/'||t.tipo||'/'||v.placa||'/'||v.codigo as nombre
			FROM esp.movimientos mov
			LEFT JOIN vehiculos v ON v.id=mov.id_vehiculo
			LEFT JOIN esp.rutas m ON m.id=mov.id_ruta
			LEFT JOIN esp.tipos_especiales t ON t.id=m.id_tipo_especial
