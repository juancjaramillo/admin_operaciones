...
if(isset($inicio)) 
{
	$inicio=date("Y-m-01", strtotime("month"));
	$final=date("Y-m-".$days_of_month,strtotime("month"));
}
...
$lineaPrimera = array("FECHA","DIA","TURNO","RUTA","HORA INICIO PLANEADA","HORA FIN PLANEADA","INICIO RUTA","FINAL RUTA","MAX HORA FIN RUTA","INCUMPLE FRECUENCIA", "INCUMPLE HORARIO");

if($html)
{
	echo '<table width="90%" border=1 bordercolor="#7fa840" class="tabla_sencilla" align="center">
			<tr>';
           ...
           FROM ( SELECT rut.id_centro,
             ...
             LEFT JOIN ( SELECT mov1.id_centro,
                   ...
                   FROM ( SELECT a.id_centro,
                          ...
                          WHERE m.final IS NOT NULL AND m.inicio::date >= '2017-01-01'::date AND s.id = 1) mov1(id_centro, id_movimiento, fecha, inicio, inicio_real, final, final_real, id_micro, esquma, id_apoyo, id_movimiento_1, inicioap, finalap, peso)
...
$consulta = " SELECT c.centro,t.turno, i.* FROM rec.indica720 i
	LEFT JOIN centros c ON c.id = i.id_centro
	LEFT JOIN turnos t ON t.id = i.id_turno
	WHERE i.id_centro = ".$centro ." AND ".$cond."
	ORDER BY fecmov,codigo";
#echo $consulta;
$qid = $db->sql_query($consulta);
while($query = $db->sql_fetchrow($qid))
{
	$dia = strftime("%u",strtotime($query["fecmov"])); 
	
	$linea = array($query["fecmov"],$dia,$query["turno"],$query["codigo"],$query["hora_inicio"],$query["hora_fin"],$query["inicio"],$query["final"],$query["maxhorafin"],$query["incumplefrec"],$query["incumplehor"]);
	
	if($html)
		imprimirLinea($linea,"", array(1=>"align='center'", 2=>"align='center'", 3=>"align='center'", 4=>"align='center'", 5=>"align='center'", 6=>"align='center'", 7=>"align='center'", 8=>"align='center'", 9=>"align='center'", 10=>"align='center'", 11=>"align='center'"));
	else
		imprimirLinea_xls($workbook, $worksheet, $fila, $columna, $linea, array(1=>"txt_center",  2=>"txt_center", 3=>"txt_center", 4=>"txt_center", 5=>"txt_center", 6=>"txt_center", 7=>"txt_center", 8=>"txt_center", 9=>"txt_center", 10=>"txt_center", 11=>"txt_center"));
}
