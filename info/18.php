...
$titulos = array("ASE","CENTRO","COSTO ASOCIADOS A<br />RECOLECCIÓN Y TRANSPORTE","TONELADAS", "COSTO POR TONELADA<br />RECOGIDA");
if($html)
{
	echo '
...
$ase = array();
$granPeso = $granViaje = $granCosto = 0;
...
{
	$ase[$ser["id"]] = array("ase"=>$ser["ase"],"centro"=>$ser["centro"], "costos"=>0, "ton"=>0, "ind"=>0);
	$vehiculos = array();
	...
	$qidMov = $db->sql_query("SELECT m.*, i.codigo, v.placa as vehiculo
			...
			WHERE i.id_ase='".$ser["id"]."' AND inicio::date >= '".$inicio."' AND inicio::date<='".$final."'");
	...
	$ase[$ser["id"]]["costos"] = costoRecoleccion($vehiculos, $inicio, $final);
	@$ase[$ser["id"]]["ind"] = $ase[$ser["id"]]["costos"]/$ase[$ser["id"]]["ton"];

	$granPeso+=$ase[$ser["id"]]["ton"];
	$granCosto+=$ase[$ser["id"]]["costos"];
	$dxGraf["data"][] =  $ase[$ser["id"]]["ind"];
	$dxGraf["labels"][] = $ser["ase"];

	$ase[$ser["id"]]["ton"]= number_format($ase[$ser["id"]]["ton"], 2, ",", ".");
	$ase[$ser["id"]]["costos"] = number_format($ase[$ser["id"]]["costos"], 2, ",", ".");
	$ase[$ser["id"]]["ind"] = number_format($ase[$ser["id"]]["ind"], 2, ",", ".");
...
else
{
	...
	$columna++;
	imprimirLineaAzul_xls($workbook, $worksheet, $fila, $columna, array( 0, number_format($granPeso, 2, ",", "."),number_format($granTotal, 2, ",", ".")));
}
...
if($html)
{
	...
	echo "</table><br /><br />";
	graficaBarras($dxGraf,"COSTO POR TONELADA RECOGIDA: ".$inicio." / ".$final, "Costo por Tonelada", "Ase", "Costo");
	echo "
