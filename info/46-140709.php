...
foreach($centros as $idCentro => $da)
{
	...
	$linea2prom[] = number_format($cosKmTOTALCen,2);
...
$linea2prom[] = number_format($cosKmTOTAL/count($centros),2);
...
if ($nivel==1 || $nivel==13 || $nivel==7 || $nivel==8 || $nivel==11 || $nivel==12 || $nivel==15){
	...
	$lineaprom = array("Costos Mtto / Costos Totales");
	...
	//ind comerciales
	$qidOV = $db->sql_query("SELECT * FROM variables_informes WHERE id=13 or id=14 or id=15 or id=17 or (id >=19 AND id<=26) or id=37 or id= 38 ORDER BY id");
	...
	$cosMtto = $gastos = $numTa = $totEmp = $totFact = $totRecau = $totRC = $totUVD = $cosMO = 0;
	foreach($centros as $idCentro => $da)
	{
		$cosMttoCen = $gastosCen = $numTaCen = $totEmpCen = $totFactCen = $totRecauCen = $totRCCen = $totUVDCen = $cosMOCen = 0;
		$vars =  array();
		$totfec = 0;
		foreach($fechas as $mes)
		{
		...
		$linea4prom[] = number_format($totEmpCen,2);
		$totEmp+=$totEmpCen;

		$totUsuCen = $totUsuCen/count($fechas);
		$linea5prom[] = number_format($totUsuCen,2);
		$totUsu+=$totUsuCen;

		$totFactCen = $totFactCen/count($lineaRecaucount);
		$lineaFactprom1[] = $totFactCen;
		$lineaFactprom[] = number_format($totFactCen,2);
		$totFact+=$totFactCen;
		...
		$lineaUVDprom[] = number_format($totUVDCen,2);
		$totUVD+=$totUVDCen;
	...
	$linea5[] = number_format($totUsu/count($centros),2);
	...
	$linea4prom[] = number_format($totEmp,2);
	$linea5prom[] = number_format($totUsu,2);
	if ($cosMO!=0)
		$linea6prom[] = number_format($cosMO/count($centros),2).'%';
	else
		$linea6prom[] ='0%';
		
	$lineaFactprom[] = number_format(array_sum($lineaFactprom1),2);
	...
	$lineaUVDprom[] = number_format($totUVD/count($centros),2);
	...
	//facturacion
	imprimirOtras($tipo_info,$fila, $columna, $html, $centros, $fechas, $nombreOtras, $otrasLineas, $estilos, array(13,14,15));
	...
	imprimirOtras($tipo_info,$fila, $columna, $html, $centros, $fechas, $nombreOtras, $otrasLineas, $estilos, array(20,21,22,23,24,25));
	if($html){
		if ($tipo_info==1){
	...
	foreach($nombreOtras as $key => $nombre)
	{
		...
		if($sigue)
		{
			...
			foreach($centros as $idCentro => $da)
			{
				...
				$lineaprom[] = number_format($totalCen,2);	
				$total+=$totalCen;
			}
			...
			if ((key($otrasLineas))==13 && ($lineaprom[0]!='Costo Facturación por usuario' && $lineaprom[0]!='Aforo a Grandes Generadores')) 		
					$lineaprom[] = number_format($total,2).$signoAd;
			else
