...
$lineaPrimera = array("CENTRO","TURNO","MACRO","INCUMPLE FRECUENCIA", "INCUMPLE HORARIO");

if($html)
{
	echo '<table width="60%" border=1 bordercolor="#7fa840" class="tabla_sencilla" align="center">
			<tr>';
           ...
           FROM ( SELECT rut.id_centro,
             ...
             LEFT JOIN ( SELECT mov1.id_centro,
                   ...
                   FROM ( SELECT a.id_centro,
                          ...
                          WHERE m.final IS NOT NULL AND m.inicio::date >= '2018-08-12'::date AND s.id = 1) mov1(id_centro, id_movimiento, fecha, inicio, inicio_real, final, final_real, id_micro, esquma, id_apoyo, id_movimiento_1, inicioap, finalap, peso)
...
if ($user["nivel_acceso"]=='3')
	$consulta = " SELECT c.centro,t.turno, substring(i.codigo from 1 for 2) as macro,
		sum(incumplefrec) as incumplefrec,
		sum(incumplehor) as incumplehor
		FROM rec.indica720 i
			LEFT JOIN centros c ON c.id = i.id_centro
			LEFT JOIN turnos t ON t.id = i.id_turno
		WHERE i.id_centro = ".$centro ." AND (".$cond." )and fecmov >= '2018-10-01'
		GROUP BY c.centro,t.turno, substring(i.codigo from 1 for 2)";
else
	$consulta = " SELECT c.centro,t.turno, substring(i.codigo from 1 for 2) as macro,
		sum(incumplefrec) as incumplefrec,
		sum(incumplehor) as incumplehor
...
{
	$lineaPrimera = array("CENTRO","TURNO","MACRO","INCUMPLE FRECUENCIA", "INCUMPLE HORARIO");
	$linea = array($query["centro"],$query["turno"],$query["macro"],$query["incumplefrec"],$query["incumplehor"]);
