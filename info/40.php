...
$lineaPrimera = array("FECHA", "TURNO", "CÓDIGO/PLACA", "COMUNA", "RUTA", "PER.CERRO", "ORDEN DE SERVICIO", "LUGAR DESCARGUE", "FECHA DE ENTRADA", "HORA ENTRADA", "FECHA DE SALIDA", "HORA SALIDA", "TIPO DE RESIDUO",  "TIPO DE SERVICIO",  "PESO DE ENTRADA", "TARA", "PESO NETO", "TICKET SALIDA");
$lineaPrimera[]=$campo;
...
$cond =" p.fecha_entrada::date >= '".$inicio."' AND p.fecha_entrada <= '".$final." 23:59:59'";
if($inicio == $final)
...
$consulta = " SELECT p.id, to_char(p.fecha_entrada,'YYYY-MM-DD') as feentrada,  to_char(p.fecha_entrada, 'HH24:MI')  as hoentrada, to_char(p.fecha_salida,'YYYY-MM-DD') as fesalida,  to_char(p.fecha_salida, 'HH24:MI')  as hosalida,  peso_inicial, peso_final, peso_total,tiquete_entrada, tiquete_salida, v.codigo||'/'||v.placa as placa, v.id as id_vehiculo, ld.nombre as lugar_descargue
	FROM rec.pesos p
...
while($query = $db->sql_fetchrow($qid))
{
	...
	$consMov = "SELECT to_char(m.inicio,'YYYY-MM-DD') as inicio, t.turno, r.comuna,  r.codigo, m.numero_orden, tr.nombre as tipo, s.servicio, p.nombre||' '||p.apellido as pcerro
		...
		WHERE mp.id_peso=".$query["id"]." AND m.id_vehiculo='".$query["id_vehiculo"]."'";
	$qidMov = $db->sql_query($consMov);
	while($mov = $db->sql_fetchrow($qidMov))
	{
//		preguntar($mov);
		$turno[$mov["turno"]] = $mov["turno"];
		$comuna[] = $mov["comuna"];
		if($mov["pcerro"] != "")
			$perCerro[] = $mov["pcerro"];
		$codigo[] = $mov["codigo"];
