...
<?
$strSQL="SELECT gid as id, id_micro, gid AS id_arco, tipo AS nombre, ST_AsGML($frm[field],7,0) as \"gmlgeom\" FROM cestas_publicas_ab WHERE $condicion ORDER BY id_micro";

$qid=$db->sql_query($strSQL);
$i=0;
$totalClases=$db->sql_field("SELECT COUNT(DISTINCT gid) FROM cestas_publicas_ab WHERE $condicion");
