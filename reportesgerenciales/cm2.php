...
$titulo1["inf"]="INDICADORES GERENCIALES : INDICADORES RECAUDO";
if($html)
{
...
if ($nivel==1 || $nivel==13 || $nivel==7 || $nivel==8 || $nivel==11 || $nivel==12 || $nivel==15){
	...
	#ind comerciales
	$qidOV = $dbnacionalproduccion->sql_query("SELECT * FROM variables_informes WHERE id=17 or id=22 or id=25 or id=26 or id=37 or id= 38 ORDER BY orden");
	while($queryOV = $dbnacionalproduccion->sql_fetchrow($qidOV))
	{
		$nombreOtras[$queryOV["id"]] = $queryOV["variable"];
	}
	
	$qidOV2 = $dbpromo->sql_query("SELECT * FROM variables_informes WHERE id=17 or id=22 or id=25 or id=26 or id=37 or id= 38 ORDER BY orden");
