...
$titulosFinanciera = array('INDICADORES DE '.$titulo1["inf"]);
...
/*=============================================================================*/
	...
	if ($tipo_info==1){
		foreach($fechas as $mes){
			$titulosFinanciera[]  = ucfirst(strftime("%b.%Y",strtotime($mes."-01")));
			$estilostitulos[$i] = " class='azul_osc_14'";
			$estilos[$i] = "class='azul_osc_12'";
			$i++;			
		}
	}else {
		$titulosFinanciera[]  = $queryCen["centro"]." Consolidado";
		$titulosFinanciera[] = "Ultimo Mes";
...
$titulosFinanciera[] = "PROMEDIOS CONSOLIDADOS";
...
$qidnivel = $dbcorporativo213->sql_query("SELECT ti.id, ti.nombre_tipo, ti.variable
...
WHERE ini.id_tipo_indicador='".$querynivel[0]."' and ini.id_nivel_indicador='".$id_nivel."' and ini.estado=1 ORDER BY ini.orden asc"); 
