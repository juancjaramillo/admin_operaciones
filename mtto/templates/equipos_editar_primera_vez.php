				...
				<tr>
					<td>
						<table width="100%" border=1 bordercolor="#7fa840" align="center">
							...
							$script = "";
							$nf=$nk=$nh=1;
							foreach($rutinas as $idRutinaPV => $dx)
							{
								echo "<tr>
									<td>".$dx["nombre"]."</td>
									<td align='center'>";
								...
								{
									?>
									<a href="javascript:copiar('fechacopy','<?=$nf?>')"><img border="0" alt="Copiar Valor Hacia Abajo" title="Copiar Valor Hacia Abajo" src="<?=$CFG->wwwroot?>/admin/iconos/transparente/flechaabajo.gif"></a>	<input type='text' size="10" id="fechacopy_<?=$nf?>" class="casillatext_fecha" name='fecha_<?=$idRutinaPV?>' value='<?=$dx["dx"]["fecha"]?>' /><button id="b_fechacopy_<?=$nf?>" onclick="javascript:showCalendarSencillo('fechacopy_<?=$nf?>','b_fechacopy_<?=$nf?>')"><img alt="Fecha" src='<?=$CFG->wwwroot?>/admin/iconos/transparente/icon-date.gif' border='0'></button>
								<?
								$nf++;
								$script.= "
									if(document.entryform.fecha_".$idRutinaPV.".value.replace(/ /g, '') !=''){
										var regexpression=/^[0-9]{4}-[0-9]{2}-[0-9]{2}/;
										var regexpressionDos=/^[+][0-9]+$/;
										if(!regexpression.test(document.entryform.fecha_".$idRutinaPV.".value) && !regexpressionDos.test(document.entryform.fecha_".$idRutinaPV.".value)){
											window.alert('La fecha de la rutina ".$dx["nombre"]." no contiene un dato válido.');
											document.entryform.fecha_".$idRutinaPV.".focus();
											return(false);
										}
										if(regexpression.test(document.entryform.fecha_".$idRutinaPV.".value)){
								...
								if($dx["horo"] != "")
								{?>
								<a href="javascript:copiar('horocopy','<?=$nh?>')"><img border="0" alt="Copiar Valor Hacia Abajo" title="Copiar Valor Hacia Abajo" src="<?=$CFG->wwwroot?>/admin/iconos/transparente/flechaabajo.gif"></a><input type='text' size='10' name='horo_<?=$idRutinaPV?>' class='casillatext' value='<?=$dx["dx"]["horometro"]?>' id='horocopy_<?=$nh?>'>
								<?
								$nh++;
								$script.= "
									if(document.entryform.horo_".$idRutinaPV.".value.replace(/ /g, '') !=''){
										var regexpression=/(^-?\d+$)|(^-?\d+\.\d+$)/;
										var regexpressionDos=/^[+][0-9]+$/;
										if(!regexpression.test(document.entryform.horo_".$idRutinaPV.".value) && !regexpressionDos.test(document.entryform.horo_".$idRutinaPV.".value)){
											window.alert('El horometro de la rutina ".$dx["nombre"]." no contiene un dato válido.');
											document.entryform.horo_".$idRutinaPV.".focus();
											return(false);
										}

										if(regexpression.test(document.entryform.horo_".$idRutinaPV.".value)){
											if(document.entryform.horo_".$idRutinaPV.".value < ".$equipo["horo"]."){
								...
								if($dx["km"] != "")
								{?>
								<a href="javascript:copiar('kmcopy','<?=$nk?>')"><img border="0" alt="Copiar Valor Hacia Abajo" title="Copiar Valor Hacia Abajo" src="<?=$CFG->wwwroot?>/admin/iconos/transparente/flechaabajo.gif"></a><input type='text' size='10' name='km_<?=$idRutinaPV?>' class='casillatext' value='<?=$dx["dx"]["km"]?>' id='kmcopy_<?=$nk?>'>
								<?
								$nk++;
								$script.= "
									if(document.entryform.km_".$idRutinaPV.".value.replace(/ /g, '') !=''){
										var regexpression=/(^-?\d+$)|(^-?\d+\.\d+$)/;
										var regexpressionDos=/^[+][0-9]+$/;
										if(!regexpression.test(document.entryform.km_".$idRutinaPV.".value) && !regexpressionDos.test(document.entryform.km_".$idRutinaPV.".value)){
											window.alert('El kilometraje de la rutina ".$dx["nombre"]." no contiene un dato válido.');
											document.entryform.km_".$idRutinaPV.".focus();
											return(false);
										}

										if(regexpression.test(document.entryform.km_".$idRutinaPV.".value)){
											if(document.entryform.km_".$idRutinaPV.".value < ".$equipo["km"]."){
